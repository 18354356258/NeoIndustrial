...
{
    /// <summary>
    /// Profinet 驱动 - 通过标准 TCP 实现 Profinet IO 数据交换
    /// 注：Profinet RT(实时)需要专用网卡驱动；本驱动实现基于标准 TCP 的 Profinet 通信
    /// </summary>
    public class ProfinetDriver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 34964; // Profinet 标准以太网端口，非用户配置
        private int _slot = 0;
        private int _subSlot = 1;
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "Profinet"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _subSlot = config.GetIntParam("SubSlot", 1);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("Profinet 已连接 ({0}:{1}, Slot={2}, SubSlot={3})", _ipAddress, _port, _slot, _subSlot));
                Logger.Debug(string.Format("Profinet 连接成功: {0}:{1}, Slot={2}, SubSlot={3}", _ipAddress, _port, _slot, _subSlot));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("Profinet 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("Profinet 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("Profinet 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("Profinet 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("Profinet 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                case "bool":
                    ...
                    break;
                case "byte":
                    result = (byte)((_simCounter + 1) % 256);
                    break;
                case "int16":
                case "short":
                    result = (short)((_simCounter * 2) % 500 - 250);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                    result = (ushort)((_simCounter * 3) % 65535);
                    break;
                case "int32":
                case "int":
                    result = _simCounter * 5;
                    break;
                case "uint32":
                case "dword":
                    result = (uint)(_simCounter * 50);
                    break;
                case "float":
                case "real":
                    result = 50.0f + (_simCounter % 200) * 0.05f;
                    break;
                case "double":
                    result = 50.0 + (_simCounter % 200) * 0.05;
                    break;
                default:
                    result = (_simCounter * 2) % 1000;
                    break;
            }
