...
{
    /// <summary>
    /// HostLink 驱动 - Omron HostLink Protocol (C-mode Commands)
    /// 通过串口(RS-232/422)或 TCP 与欧姆龙 PLC 通信
    /// </summary>
    public class HostLinkDriver : IDriver
    {
        ...
        public string DriverType { get { return "HostLink"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _unitNo = config.GetIntParam("UnitNo", 0);
            _transport = config.GetParam("Transport", "TCP");
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("HostLink 已连接 ({0}:{1}, 单元号{2}, {3})",
                    _ipAddress, _port, _unitNo, _transport));
                Logger.Debug(string.Format("HostLink 连接成功: {0}:{1}, 单元号{2}", _ipAddress, _port, _unitNo));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("HostLink 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("HostLink 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("HostLink 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("HostLink 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("HostLink 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            // TODO: 实现真正的 HostLink C-mode 命令读取
            // 当前返回模拟数据用于驱动框架验证
            switch (point.DataType.ToLower())
            {
                case "bool":
                case "coil":
                    result = (_simCounter % 5 == 0);
                    break;
                case "byte":
                    result = (byte)((_simCounter + 20) % 256);
                    break;
                case "int16":
                case "short":
                    result = (short)((_simCounter * 2) % 200 - 100);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                    result = (ushort)((_simCounter * 6) % 65535);
                    break;
                case "int32":
                case "int":
                    result = _simCounter * 4;
                    break;
                case "uint32":
                case "dword":
                    result = (uint)(_simCounter * 40);
                    break;
                case "float":
                case "real":
                    result = 20.0f + (_simCounter % 60) * 0.3f;
                    break;
                case "double":
                    result = 20.0 + (_simCounter % 60) * 0.3;
                    break;
                default:
