...
{
    /// <summary>
    /// DNP3 驱动 - 通过 DNP3 over TCP 与电力远动设备通信
    /// 标准端口 20000
    /// </summary>
    public class DNP3Driver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 20000;
        private int _sourceAddress = 1;
        private int _destAddress = 100;
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "DNP3"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _port = config.GetIntParam("Port", 20000);
            _sourceAddress = config.GetIntParam("SourceAddress", 1);
            _destAddress = config.GetIntParam("DestAddress", 100);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("DNP3 已连接 ({0}:{1}, Src={2}, Dst={3})", _ipAddress, _port, _sourceAddress, _destAddress));
                Logger.Debug(string.Format("DNP3 连接成功: {0}:{1}, Src={2}, Dst={3}", _ipAddress, _port, _sourceAddress, _destAddress));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("DNP3 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("DNP3 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("DNP3 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("DNP3 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("DNP3 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                case "bool":
                    ...
                    break;
                case "byte":
                    result = (byte)((_simCounter + 7) % 256);
                    break;
                case "int16":
                case "short":
                    result = (short)((_simCounter * 4) % 800 - 400);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                ...
                case "int32":
                case "int":
                    result = _simCounter * 6;
                    break;
                case "uint32":
                case "dword":
                    result = (uint)(_simCounter * 60);
                    break;
                case "float":
                case "real":
                    result = 50.0f + (_simCounter % 80) * 0.25f;
                    break;
                case "double":
                    result = 50.0 + (_simCounter % 80) * 0.25;
                    break;
                default:
