...
{
    /// <summary>
    /// SECS/GEM 驱动 - 通过 HSMS (High-Speed SECS Message Services) over TCP 与半导体设备通信
    /// 标准端口 5000
    /// </summary>
    public class SecsGemDriver : IDriver
    {
        ...
        public string DriverType { get { return "SecsGem"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _port = config.GetIntParam("Port", 5000);
            _deviceId = config.GetIntParam("DeviceID", 1);
            _sessionId = config.GetIntParam("SessionID", 0);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("SECS/GEM 已连接 ({0}:{1}, DevID={2}, Session={3})", _ipAddress, _port, _deviceId, _sessionId));
                Logger.Debug(string.Format("SECS/GEM 连接成功: {0}:{1}, DevID={2}, Session={3}", _ipAddress, _port, _deviceId, _sessionId));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("SECS/GEM 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("SECS/GEM 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("SECS/GEM 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("SECS/GEM 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("SECS/GEM 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                ...
                case "int16":
                case "short":
                    result = (short)((_simCounter * 6) % 1200 - 600);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                    result = (ushort)((_simCounter * 15) % 65535);
                    break;
                case "int32":
                case "int":
                ...
                case "float":
                case "real":
                    result = 100.0f + (_simCounter % 120) * 0.4f;
                    break;
                case "double":
                    result = 100.0 + (_simCounter % 120) * 0.4;
                    break;
                default:
                    result = (_simCounter * 6) % 1000;
                    break;
            }
