...
{
    /// <summary>
    /// FINS 驱动 - Omron FINS Protocol
    /// 通过 UDP 或 TCP 与欧姆龙 PLC 通信
    /// </summary>
    public class FinsDriver : IDriver
    {
        ...
        private int _sourceUnit = 0;
        private int _destUnit = 0;
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "Fins"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _transport = config.GetParam("Transport", "UDP");
            _sourceNode = config.GetIntParam("SourceNode", 0);
            _destNode = config.GetIntParam("DestNode", 1);
            _sourceUnit = config.GetIntParam("SourceUnit", 0);
            _destUnit = config.GetIntParam("DestUnit", 0);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("FINS 已连接 ({0}:{1}, {2}, 源节点{3} 目标节点{4})",
                    _ipAddress, _port, _transport, _sourceNode, _destNode));
                Logger.Debug(string.Format("FINS 连接成功: {0}:{1}, {2}", _ipAddress, _port, _transport));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("FINS 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("FINS 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("FINS 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("FINS 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("FINS 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                ...
                case "int16":
                case "short":
                    result = (short)((_simCounter * 4) % 200 - 100);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                    result = (ushort)((_simCounter * 8) % 65535);
                    break;
                case "int32":
                case "int":
                ...
                case "float":
                case "real":
                    result = 30.0f + (_simCounter % 80) * 0.25f;
                    break;
                case "double":
                    result = 30.0 + (_simCounter % 80) * 0.25;
                    break;
                default:
