...
{
    /// <summary>
    /// MELSEC MC 驱动 - Mitsubishi MELSEC MC Protocol (3E/4E Frame)
    /// 通过 TCP 与三菱 MELSEC 系列 PLC 通信
    /// </summary>
    public class MELSECMcDriver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 5000;
        private string _frameType = "3E";
        ...
        public string DriverType { get { return "MELSECMc"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _frameType = config.GetParam("FrameType", "3E");
            _networkNo = config.GetIntParam("NetworkNo", 0);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("MELSEC MC 已连接 ({0}:{1}, {2}帧, 网络{3} 站{4})",
                    _ipAddress, _port, _frameType, _networkNo, _stationNo));
                Logger.Debug(string.Format("MELSEC MC 连接成功: {0}:{1}, {2}帧", _ipAddress, _port, _frameType));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("MELSEC MC 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("MELSEC MC 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("MELSEC MC 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("MELSEC MC 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("MELSEC MC 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            // TODO: 实现真正的 MELSEC MC 3E/4E 帧批量读取
            // 当前返回模拟数据用于驱动框架验证
            switch (point.DataType.ToLower())
            {
                ...
                case "int16":
                case "short":
                    result = (short)(_simCounter % 200 - 100);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                ...
                case "float":
                case "real":
                    result = 25.0f + (_simCounter % 100) * 0.15f;
                    break;
                case "double":
                    result = 25.0 + (_simCounter % 100) * 0.15;
                    break;
                default:
