...
{
    /// <summary>
    /// KNX 驱动 - 通过 KNXnet/IP over TCP 与楼宇自动化设备通信
    /// 标准端口 3671
    /// </summary>
    public class KNXDriver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 3671;
        private string _individualAddress = "1.1.0";
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "KNX"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _port = config.GetIntParam("Port", 3671);
            _individualAddress = config.GetParam("IndividualAddress", "1.1.0");
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("KNX 已连接 ({0}:{1}, IA={2})", _ipAddress, _port, _individualAddress));
                Logger.Debug(string.Format("KNX 连接成功: {0}:{1}, IA={2}", _ipAddress, _port, _individualAddress));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("KNX 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("KNX 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("KNX 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("KNX 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("KNX 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                ...
                case "float":
                case "real":
                    result = 21.0f + (_simCounter % 40) * 0.15f;
                    break;
                case "double":
                    result = 21.0 + (_simCounter % 40) * 0.15;
                    break;
                default:
