...
{
    /// <summary>
    /// IEC 61850 驱动 - 通过 MMS (Manufacturing Message Specification) over TCP 与变电站设备通信
    /// 标准端口 102
    /// </summary>
    public class IEC61850Driver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 102;
        private string _domain = "IED";
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "IEC61850"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _domain = config.GetParam("Domain", "IED");
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("IEC61850 已连接 ({0}:{1}, Domain={2})", _ipAddress, _port, _domain));
                Logger.Debug(string.Format("IEC61850 连接成功: {0}:{1}, Domain={2}", _ipAddress, _port, _domain));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("IEC61850 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("IEC61850 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("IEC61850 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("IEC61850 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("IEC61850 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            // TODO: 实现真正的 IEC61850 MMS Read 请求
            // 当前返回模拟数据用于驱动框架验证
            switch (point.DataType.ToLower())
            {
                ...
                case "float":
                case "real":
                    result = 110.0f + (_simCounter % 100) * 0.3f;
                    break;
                case "double":
                    result = 110.0 + (_simCounter % 100) * 0.3;
                    break;
                default:
