...
{
    /// <summary>
    /// LonWorks 驱动 - 通过 LonWorks/IP over TCP 与楼宇自控设备通信
    /// 标准端口 2540
    /// </summary>
    public class LonWorksDriver : IDriver
    {
        private TcpClient _tcpClient;
        private NetworkStream _stream;
        private string _ipAddress = "127.0.0.1";
        private int _port = 2540;
        private int _networkId = 0;
        private DeviceConfig _config;
        private bool _disposed;
        private int _simCounter;

        public string DriverType { get { return "LonWorks"; } }
        ...
        public async Task<bool> ConnectAsync(DeviceConfig config)
        {
            ...
            _port = config.GetIntParam("Port", 2540);
            _networkId = config.GetIntParam("NetworkID", 0);
            ...
            try
            {
                ...
                IsConnected = true;
                NotifyStatus(true, string.Format("LonWorks 已连接 ({0}:{1}, NetID={2})", _ipAddress, _port, _networkId));
                Logger.Debug(string.Format("LonWorks 连接成功: {0}:{1}, NetID={2}", _ipAddress, _port, _networkId));
                return true;
            }
            catch (Exception ex)
            {
                ...
                Logger.Warn(string.Format("LonWorks 连接失败 [{0}]: {1}", _config.Name, ex.Message));
                return false;
            }
        }
        ...
        public async Task StartCollectAsync(CancellationToken token)
        {
            ...
            Logger.Debug(string.Format("LonWorks 采集开始: {0}, IP={1}:{2}, 间隔={3}ms",
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var point in _config.DataPoints)
                    {
                        ...
                        {
                            Logger.Debug(string.Format("LonWorks 读取失败 [{0}.{1}]: {2}", _config.Name, point.Name, ex.Message));
                        }
                ...
                {
                    IsConnected = false;
                    Logger.Warn(string.Format("LonWorks 采集异常 [{0}]: {1}", _config.Name, ex.Message));
                    await Task.Delay(1000, token);
                }
            }

            Logger.Debug(string.Format("LonWorks 采集结束: {0}", _config.Name));
        }
        ...
        public Task<object> ReadAsync(DataPoint point)
        {
            ...
            switch (point.DataType.ToLower())
            {
                case "bool":
                    ...
                    break;
                case "byte":
                    result = (byte)((_simCounter + 12) % 256);
                    break;
                case "int16":
                case "short":
                    result = (short)((_simCounter * 3) % 600 - 300);
                    break;
                case "uint16":
                case "ushort":
                case "word":
                    result = (ushort)((_simCounter * 13) % 65535);
                    break;
                case "int32":
                case "int":
                ...
                case "float":
                case "real":
                    result = 25.0f + (_simCounter % 60) * 0.35f;
                    break;
                case "double":
                    result = 25.0 + (_simCounter % 60) * 0.35;
                    break;
                default:
