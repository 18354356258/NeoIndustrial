...
**39 种工业协议驱动 · 五类数据库并行落库 · MQTT 毫秒级上云 · 50 个 MCP AI 工具 · 语义层数字孪生**
...
- **用得活**：MQTT 双层话题上云、REST API 对接第三方、50 个 MCP 工具让 AI 用自然语言管理设备与变量、语义层数字孪生建模、Fabric 时序分析、Dashboard 看板、配置模板与设备克隆批量部署。
...
| 🤖 **MCP AI 集成** | 内置 MCP Server（50 个工具），Claude Desktop 等 AI 客户端连接后可用自然语言添加设备、配置变量、启停采集、查数据、生成日报 |
...
> ⚠️ **关于需要授权的驱动（请务必先读）**
>
> 带 🔑 标记的驱动（Fanuc FOCAS、Heidenhain、Siemens 840D、SECS/GEM、OPC DA、HART IP、EtherNet/IP、DeviceNet、CC-Link、KNX、DLMS/COSEM、LonWorks），其规范、SDK 或运行库受**原厂或标准组织的商业授权 / 会员资格**约束。**用于生产、商用或对外交付前，请先联系对应厂家或授权机构，购买正版授权、取得正式许可后再使用**；未取得授权时，请仅用于学习、研究与测试评估。
>
> 其余驱动（Modbus、Siemens S7、三菱 MC/FX、基恩士 KV、松下 Mewtocol、欧姆龙 FINS/HostLink、倍福 ADS、CODESYS、Haas、Mazak、MTConnect、PROFIBUS、PROFINET、BACnet、OPC UA、IEC 104、DNP3、DALI、M-Bus、MQTT、Sparkplug B、HTTP REST 等）为公开协议实现，通常无需向厂商购买授权即可实现；个别协议的**规范文档本身**需付费购买或仅对会员开放，但仅需取得规范即可自行实现。**是否需授权最终以原厂与标准组织现行条款为准。**
>
> 本软件只实现协议对接，**不附带、不代购、不破解、不分发任何厂商 SDK、授权文件、授权码或功能选件**。使用者须自行确认并取得所用设备与协议所需的全部授权和许可；因未取得授权而使用相关协议所产生的责任与纠纷由使用者自行承担。
...
- 39 种协议驱动、五类数据库并行写入、MQTT 双层话题、50 个 MCP 工具、REST API、语义层建模、Fabric 分析、看板、模板与克隆、软件激活模块；
