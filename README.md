...
--- 下载 企业版 客户端 在下方 五分钟上手位置处

## 🎨 这套数采软件的前世今生

2026 年春天，我在家开发字孪生系统的 UI 原型和驱动逻辑。

我不是计算机科班出身。我是个**艺术生** —— 色彩、光影、构图才是我的语言。但数字孪生的世界里，你光画得好看没用——没有真实数据灌进去，再华丽的 3D 模型也只是个精致的空壳。

我的铝挤压数字孪生项目已经落地，目前疯狂迭代中，三语（中英越），带AI分析，温度趋势看板，挤压周期自动落数据库，数据导出，秒级数据同步，毫米级模型位移，实时对应生产现场设备运转（未来有可能也会开源，敬请期待）。

开发数字孪生系统时，我需要能连上工厂里每一台设备的东西。 PLC、CNC、传感器、仪表、电力柜、楼宇控制器……它们说着完全不同的语言：Modbus、S7、FOCAS、BACnet、IEC 61850……市面上的方案要么贵到离谱（一套 Kepware 够买一台机床），要么重到工控机根本跑不动（装个 Ignition 比造产线还慢），要么老旧到连 AI 的影子都看不见。

**没找到能用的。于是我放下了画笔，打开了 Visual Studio。**

（背景：毕业后就进钢铁厂，目前在铝挤加工行业3年，一共干了12年IT）

不到三个月，一个人——一个艺术生——从零写出了 40 个工业协议驱动、4 种数据库并行写入、MQTT 双层毫秒推送、50 个 MCP AI 原子工具、REST API、Dashboard 监控大屏。不是为了证明什么，只是真的需要一副能连接一切工业设备的神经末梢，让数字孪生活起来了。

**这不是又一个"工业软件"。这是一个艺术生为数字孪生世界打造的数据基座。**

现在，我把社区版完整开源——Apache 2.0，永久的、真正免费的。

因为我知道：在中国做开源的工业软件有多不容易。一个人走在深夜的车间里，只有 PLC 的指示灯在闪的时候，你需要的不是更多的门槛和收费，而是一个能用的、真诚的、不设防的工具。

希望你喜欢。希望它有用，希望能助力想从美术设计前端往IT转型的朋友们，真心希望这个软件能够帮到你们。

---
![登陆界面](picture/%E7%99%BB%E5%BD%95%E9%A1%B5.png)
...
IndustrialDataCollector 是一套**生产级工业数据采集引擎 ** — 已经在生产现场跑着的稳如老狗的

从冲压车间的 PLC 到海上风场的 IEC 61850 变电站，从 CNC 五轴加工中心的 Fanuc 控制器到半导体 SECS/GEM 机台

**一套代码全部通吃**

40 种驱动、4 种数据库、MQTT 双层话题推送、50 个 MCP AI 原子工具，零门槛启动，无加密无认证无 License。

**社区版 Apache 2.0 开源，永久免费。** 

你只需要把时间花在工艺优化和业务场景分析上，而不是在"谁能连接上谁"的破事上。
...
企业版客户端获取：发送需求 `751326339@qq.com`，或致电 `18354356258` / `18854344113`（张成龙）获取。
...
© 2026 张成龙 · 社区版 Apache 2.0 · 企业版商业许可 需联系开发者
